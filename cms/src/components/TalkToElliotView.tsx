...
const AGENTS: Agent[] = [
  {
    ...
    skills: [
      ...
      { status: "live",       name: "maintenance-pass", signature: "maintenance-pass([--apply] [--news-days=N] [--feature-days=N])", desc: "Scan published articles, find stale Events (> 14d), News (> 30d default), Features (> 180d default). Dry-run by default; --apply flips expired Events to draft." },
    ],
  ...
  },
  {
    ...
    skills: [
      ...
      { status: "live",       name: "persona-check",     signature: "persona-check(--id [--persona=...])", desc: "Score voice match 0–10 via Vertex Gemini structured output. Returns score + verdict (publish-ready/minor-edits/needs-rewrite/not the persona) + summary + 3–5 issues + 3–5 concrete rewrite suggestions." },
    ],
  ...
  },
  {
    ...
    skills: [
      ...
      { status: "live",       name: "competitor-gap",   signature: "POST /api/seo-competitor-gap {area, topic, missing_themes[]}", desc: "Takes a gap-report from the Crawler and ranks each missing theme by SEO opportunity. Returns ranked_gaps with primary_keyword, long_tail, search_potential (high/med/low), suggested_brief ready for dispatch, angle, and rank." },
    ],
  ...
  },
  {
    ...
    skills: [
      ...
      { status: "live",       name: "regenerate",      signature: "regenerate(--id, --feedback)", desc: "Wrapper around generate-hero with feedback-augmented prompt. Smart negative-prompt mapping (\"no people\" → adds people/faces). Uploads new PNG to GCS, returns old + new media ids for the caller to PATCH article.hero." },
