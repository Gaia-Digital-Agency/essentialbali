...
const MenuNav: React.FC<{ menu: Category; menus: Category[] }> = ({ menu, menus }) => {
  ...
  const isActive = () => {
    ...
    if (actualRoute.category?.id_parent && !menus.find((men) => actualRoute.category?.id == men.id)) {
      return taxonomies.categories?.find((cat) => actualRoute.category?.id_parent == cat.id)?.id == menu.id
    }
    return false
  }
...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header className="relative top-0 left-0 right-0 z-[100] bg-white" role="banner">
        <div className="container mx-auto py-5 flex justify-between items-center gap-4">
          <div className="logo-wrapper w-max flex items-center gap-3">
            ...
            <span className="hidden md:block text-front-body-big text-front-red uppercase tracking-[0.08em]">essentialbali</span>
          </div>
        ...
        <div className="container mx-auto py-4 hidden md:flex items-center gap-4 area-nav-tools">
          ...
          <form onSubmit={areaSearchSubmitHandler} className="area-search-wrapper flex-1 flex items-center gap-2">
            <input
              type="text"
            ...
            <button type="submit" className="bg-front-red text-white uppercase px-5 py-3 text-front-small font-semibold">
              Go
            </button>
          </form>
        </div>
        ...
        <div className="container mx-auto py-4 hidden md:block">
          <nav className="menus-wrapper flex flex-wrap items-center gap-x-6 gap-y-3" aria-label="Categories">
            {forcedMenuCategories.map((menu: Category) => (
              <MenuNav menu={menu} menus={forcedMenuCategories} key={`header-menu-${menu.slug_title}`} />
            ))}
          </nav>
        </div>
      ...
      <MobileMenu isModalOpen={isModalOpen} closeModal={() => setIsModalOpen(false)} />
    </>
  )
}
